...
public class BaiDuMapTrackActivity extends AppCompatActivity {
    ...
    private List<LatLng> runPoints = new ArrayList<>();//绘制的大点集合
    ...
    private ImageButton imgBtnBack, btnStart, btnStop, imageButton, btnEarth;//开始停止信息
    ...
    private final int move_icon = R.mipmap.i2;//移动图标
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        imgBtnBack = (ImageButton) findViewById(R.id.imageButton);
        ...
        imgBtnBack.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Log.e(TAG, "onClick: imgBtnBack = " + imgBtnBack);
            }
        });
    }
    ...
    private void initDialog() {
        ...
        new AlertDialog.Builder(BaiDuMapTrackActivity.this)
                .setView(view)
                .setMessage("回放选项")
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (count > 1440) {
                            Toast.makeText(getApplicationContext(), "相差不能大于24小时", Toast.LENGTH_LONG).show();
                        } else {
                            mBaiduMap.clear();
                            runLocal = getData();
                            if (showType == 1) {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.txt_map_start_time: {
                    ...
                    DatePickerDialog dialog = new DatePickerDialog(BaiDuMapTrackActivity.this, new DatePickerDialog.OnDateSetListener() {
                        @Override
                        public void onDateSet(DatePicker view, final int years, final int monthOfYear, final int dayOfMonth) {
                            new TimePickerDialog(BaiDuMapTrackActivity.this,
    ...
    Marker move_icon_marker;//待移动的那个marker图标
    int cav_index = 0;
    List<LatLng> list_cav = new ArrayList<>();
    private Polyline main_polyline = null;//大点之间的线段
    private Polyline cav_polyline = null;//切割的小点之间的线段
    ...
    private void startWire() {
        runBitmap.add(BitmapDescriptorFactory.fromAsset("icon_road_blue_arrow.png"));
        runBitmap.add(BitmapDescriptorFactory.fromAsset("icon_road_green_arrow.png"));
        runBitmap.add(BitmapDescriptorFactory.fromAsset("icon_road_blue_arrow.png"));
        runBitmap.add(BitmapDescriptorFactory.fromAsset("icon_road_red_arrow.png"));
        onClickMarker();
        setLatLng();
        move_icon_marker = (Marker) mBaiduMap.addOverlay(
                new MarkerOptions().position(runPoints.get(0)).icon(BitmapDescriptorFactory.fromResource(move_icon)));
    ...
    private void cav(final List<LatLng> latLng, final int color_index) {
        ...
        if (latLng.size() != 0)
            list_cav.add(latLng.get(0));//添加小线段起点
        //添加小线段颜色
        final List<Integer> cav_polyline_color = new ArrayList<>();
        cav_polyline_color.add(color_index);
        ...
        TimerTask task = new TimerTask() {
            ...
            public void run() {
                ...
                if (cav_index == latLng.size() || latLng.size() == 0) {
                    ...
                    //mBaiduMap.clear();
                    //mBaiduMap.setMapStatus(MapStatusUpdateFactory.newLatLng(runPoints.get(index)));//定位显示到初始地点
                    Log.e(TAG, "大点之间的线段: main_polyline = " + main_polyline);
                    if (main_polyline != null) main_polyline.remove();

                    //TODO 画大点线段(这里是画options.size()个大点之间的线段)
                    main_polyline = (Polyline) mBaiduMap.addOverlay(
                            new PolylineOptions().width(10).points(options).dottedLine(true)
                                    .customTextureList(runBitmap).textureIndex(list_int));
                    Log.d(TAG, "开始画大线 index是从零开始计算的 index = " + index);
                            ...
                            if (timeMin > 0) {
                                if (isDot) {
                                    ...
                                    MarkerOptions options = new MarkerOptions()
                                            .position(getLatLng(runLocal.get(index))).icon(BitmapDescriptorFactory.fromResource(resource));
                    ...
                    // 更新move_icon_marker的位置
                    move_icon_marker.setPosition(latLng.get(cav_index));
                    Log.e(TAG, "run: 画小线段 list_cav = " + list_cav.size());
                    if (cav_polyline == null) {
                        // 首次画线
                        cav_polyline = (Polyline) mBaiduMap.addOverlay(
                                new PolylineOptions().width(10).points(list_cav).dottedLine(true)
                                        .customTextureList(runBitmap).textureIndex(cav_polyline_color));
                    } else {
                        // 更新线段
                        cav_polyline.setPoints(list_cav);
                    }
                    // list_cav里面永远只有两个点，这两个点组成一小段分割线
                    // remove(1)的目的是为了，更新起点经纬度
                    list_cav.remove(1);
                    cav_index++;
                }
            }
                ...
                if (STATE == 1) {
                    ...
                    TimerTask task = new TimerTask() {
                        ...
                        public void run() {
                            ...
                            // 画Maeker并填充数据
                            Marker marker1 = (Marker) mBaiduMap.addOverlay(
                                    new MarkerOptions().position(latLng).icon(BitmapDescriptorFactory.fromResource(resource)));
                            Bundle bundle1 = new Bundle();
                            bundle1.putInt("index", index);
                            marker1.setExtraInfo(bundle1);
                            markerList.add(marker1);
                            // 确保最后一个marker的图标是move_icon
                            // 调用marker.setIcon()后，地图会自动更新marker的图标
                            markerList.get(index).setIcon(BitmapDescriptorFactory.fromResource(move_icon));

                            if (index > 0) {
                                resource = getGPStoImage(localType(runLocal.get(index - 1).getSource()));
                                try {
                                    if (timeMin > 0 && index > 1
                                            && getDateMin(runLocal.get(index - 2).getUpdatetime(), runLocal.get(index - 1).getUpdatetime()) > 1) {
                                        resource = R.mipmap.ic_pin_red;
                                    }
                                } catch (ParseException e) {
                                    e.printStackTrace();
            ...
            public boolean onMarkerClick(final Marker marker) {
                ...
                geoCoder.setOnGetGeoCodeResultListener(new OnGetGeoCoderResultListener() {
                    ...
                    public void onGetReverseGeoCodeResult(ReverseGeoCodeResult arg0) {
                        ...
                        txtDate.setText(runLocal.get(finalIndexMarker).getUpdatetime()
                                + " " + localType(runLocal.get(finalIndexMarker).getSource()));
                        if (timeMin > 0) {
    ...
    private List<LocalBean> getData() {
        ...
        for (LocalBean l : JsonUtil.jsonToList(json, LocalBean.class)) {
            switch (LOCAL_TYPE) {
                case 0: {
                    list.add(l);
                    break;
